...
def _shopee_extrair_ids(link):
    """Tenta achar shopId e itemId em qualquer um dos formatos que a
    Shopee ja usou:
      - .../produto-i.SHOPID.ITEMID  (formato antigo)
      - .../product/SHOPID/ITEMID    (formato atual, 2026)
    Se for um link curto (shope.ee/... ou s.shopee.com.br/...), segue o
    redirecionamento primeiro pra achar a URL completa."""
    ids = _shopee_tentar_padroes(link)
    if ids:
        return ids
    ...
    try:
        ...
        ids = _shopee_tentar_padroes(resposta.url)
        if ids:
            return ids
...
def _shopee_tentar_padroes(url):
